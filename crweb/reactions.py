...
class Reaction(object):
  ...
  def exists(self, name, uid, rdb):
    ''' This will check to see if the specified reactions already exists or not '''
    result = r.table('reactions').filter({'name': name, 'uid': uid}).count().run(rdb)
    if result >= 1:
      return True
    else:
      return False
  ...
  def getRID(self, searchstring, rdb):
    ...
    result = r.table('reactions').filter({'name': strings[0], 'uid': strings[1]}).run(rdb)
    xdata = {}
    for x in result:
  ...
  def get(self, method, lookup, rdb):
    ''' This will return a reactions information based on the data provided '''
    if method == 'rid':
      rid = lookup
    else:
