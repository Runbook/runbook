...
class ReactForm(BaseReactForm):
    ...
    dropletid = TextField("Droplet ID#", validators=[DataRequired(message='Droplet ID# is a required field'), NumberRange(
        min=1, max=None, message="Droplet ID should be a numeric ID number")])
    snapname = TextField("Snapshot Name", validators=[
                         DataRequired(message='Snapshot Name is a required field')])
