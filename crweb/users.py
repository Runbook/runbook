...
class User(object):
  ...
  def setPass(self, newpass, rdb):
    ...
    results = r.table('users').get(self.uid).update({'password': password}).run(rdb)
  ...
  def getUID(self, username, rdb):
      ...
      results = r.table('users').filter(r.row['username'] == username).run(rdb)
      xdata = {}
      for x in results:
        key = x['username']
        value = x['id']
  ...
  def is_active(self, username, rdb):
    ...
    count = r.table('users').filter({ 'username': username }).count().run(rdb)
    if count >= 1:
      self.active = True
    else:
      self.active = False
  ...
  def getReactions(self, rdb):
    ...
    ## Get Reactions
    results = r.table('reactions').filter({'uid': self.uid}).order_by('name').run(rdb)
    reactions = {}
    for x in results:
      reactions[x['id']] = x
  ...
  def getMonitors(self, rdb):
    ...
    ## Get Monitors
    results = r.table('monitors').filter({'uid': self.uid}).order_by('name').run(rdb)
    monitors = {}
    for x in results:
     monitors[x['id']] = x
  ...
  def getEvents(self, rdb):
    ...
    ## Get Events
    results = r.table('events').filter({'uid': self.uid}).order_by(r.desc('time')).limit(50).run(rdb)
    return results
