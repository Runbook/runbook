...
def before_request():
  ...
  try:
    g.rdb_conn = r.connect( host=app.config['DBHOST'], port=app.config['DBPORT'], auth_key=app.config['DBAUTHKEY'], db=app.config['DATABASE'])
    ...
    if user.status != "active":
      ...
      for key in data['monitors'].keys():
        if "healthy" in data['monitors'][key]['status']:
          data['monstats']['healthy'] = data['monstats']['healthy'] + 1
        elif "failed" in data['monitors'][key]['status']:
          data['monstats']['failed'] = data['monstats']['failed'] + 1
        else:
          data['monstats']['unknown'] = data['monstats']['unknown'] + 1
...
def addcheck_page(cname):
  ...
  if verify:
    ...
    if user.status != "active":
      ...
      data['reactions'] = user.getReactions(g.rdb_conn)
      ## Proces the form
      cform = __import__("monitorforms." + cname, globals(), locals(), ['CheckForm'], -1)
      form = cform.CheckForm(request.form)
      if form.__contains__("timer"):
        form.timer.choices = data['choices']
      reactchoices = []
      for key in data['reactions'].keys():
        reactchoices.append((data['reactions'][key]['id'], data['reactions'][key]['name']))
      ...
      if request.method == 'POST':
        if form.validate():
          ...
          if results == "exists":
            ...
            stathat.ez_count(app.config['STATHAT_EZ_KEY'], app.config['ENVNAME'] + ' Too many health checks', 1)
...
def editcheck_page(cname, cid):
  ...
  if verify:
    ...
    if user.status != "active":
      ...
      for key in data['reactions'].keys():
        reactchoices.append((data['reactions'][key]['id'], data['reactions'][key]['name']))
        if data['reactions'][key]['id'] in data['monitor']['data']['reactions']:
...
## Web Checks
@app.route('/api/<atype>/<cid>', methods=['POST'], defaults={'check_key': None, 'action': None})
@app.route('/api/<atype>/<cid>/<check_key>', methods=['POST'], defaults={'action': None})
  ...
  if verify:
    ...
    if user.status != "active":
      ...
      if monitor.uid == user.uid:
        ...
        data['monitor-history-count'] = monitor.history(method="count", time=chktime, rdb=g.rdb_conn)
        data['monitor-history'] = monitor.history(method="mon-history", time=chktime, start=int(start), limit=int(limit), rdb=g.rdb_conn)
        data['error'] = False
  ...
  if verify:
    ...
    if user.status != "active":
      ...
      if monitor.uid == user.uid:
        ...
        data['monitor-history'] = monitor.history(method="detail-history", hid=hid, rdb=g.rdb_conn)
        data['error'] = False
      else:
        data['msg'] = "This monitor does not belong to your user"
        data['error'] = True
...
def delreaction_page(rid):
  ''' Dashboard Delete Domains: This will delete a domain based on url parameters'''
  verify = verifyLogin(app.config['SECRET_KEY'], app.config['COOKIE_TIMEOUT'], request.cookies)
  if verify: 
    user = User()
    user.get('uid', verify, g.rdb_conn)
    if user.status != "active":
      ...
      appliedcount = 0
      results = r.table('monitors').filter({'uid': user.uid}).run(g.rdb_conn)
      for x in results:
