  ...
  for qitem in results:
    ...
    if qitem['type'] == "monitor":
      ...
      if qitem['action'] == "delete" or qitem['action'] == "edit":
        line = "Initiating Monitor deletion for monitor id: %s" % qitem['item']['cid']
        syslog.syslog(syslog.LOG_DEBUG, line)
        result = decimateRedis(keyid, qitem['item'])
        if result:
          line = "Monitor %s removed redis queue" % qitem['item']['cid']
          syslog.syslog(syslog.LOG_INFO, line)
          if qitem['action'] == "delete":
            delete = r.table(config['dbqueue']).get(qitem['id']).delete().run(rdb_server) 
            if delete['deleted'] == 1:
      ...
      if qitem['action'] == "create" or qitem['action'] == "edit":
        if "datacenter" not in qitem['item']['data']:
          ...
          if config['dbqueue'] in qitem['item']['data']['datacenter']:
            ...
            result = populateRedis(keyid, qitem['item'], local=True)
          else:
            line = "Initiating Monitor creation for monitor id: %s - notify" % qitem['item']['cid']
            result = populateRedis(keyid, qitem['item'])
        syslog.syslog(syslog.LOG_DEBUG, line)
        if result:
          line = "Monitor %s added to redis queue" % qitem['item']['cid']
          syslog.syslog(syslog.LOG_INFO, line)
          delete = r.table(config['dbqueue']).get(qitem['id']).delete().run(rdb_server) 
          if delete['deleted'] == 1:
            line = "Queue entry %s removed from RethinkDB queue" % qitem['id']
            syslog.syslog(syslog.LOG_DEBUG, line)
            status = r.table('monitors').get(qitem['item']['cid']).update({ 'status': 'monitored' }).run(rdb_server)
    ...
    if qitem['type'] == "reaction":
      ...
      if qitem['action'] == "delete" or qitem['action'] == "edit":
        ...
        result = decimateRedis(keyid, qitem['item'])
        if result:
          line = "Reaction %s removed from redis" % qitem['item']['rid']
          syslog.syslog(syslog.LOG_INFO, line)
          delete = r.table(config['dbqueue']).get(qitem['id']).delete().run(rdb_server) 
          if delete['deleted'] == 1:
            line = "Queue entry %s removed from RethinkDB queue" % qitem['id']
