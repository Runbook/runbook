...
## Redis Server
try:
  r_server = redis.Redis(host=config['redis_host'], port=config['redis_port'], db=config['redis_db'],password=config['redis_password']) 
...
## RethinkDB Server
try:
  rdb_server = r.connect( host=config['rethink_host'], port=config['rethink_port'], auth_key=config['rethink_authkey'], db=config['rethink_db'])
...
def runAction(redata, jdata):
  ...
  if "failed" in jdata['check']['status']:
    reaction = __import__("actions." + redata['rtype'], globals(), locals(), ['failed'], -1)
    react = reaction.failed(redata, jdata, rdb_server, r_server)
    if react == True:
      line = "Successfully processed failed reaction type %s for monitor %s" % (redata['rtype'], jdata['cid'])
      syslog.syslog(syslog.LOG_INFO, line)
      return True
    elif react == None:
      line = "Skipped failed reaction type %s for monitor %s" % (redata['rtype'], jdata['cid'])
      syslog.syslog(syslog.LOG_INFO, line)
      return None
    else:
      line = "Processing reaction type %s for monitor %s did not occur" % (redata['rtype'], jdata['cid'])
      syslog.syslog(syslog.LOG_INFO, line)
      return False
  elif "healthy" in jdata['check']['status']:
    reaction = __import__("actions." + redata['rtype'], globals(), locals(), ['healthy'], -1)
    react = reaction.healthy(redata, jdata, rdb_server, r_server)
    if react:
      line = "Successfully processed healthy reaction type %s for monitor %s" % (redata['rtype'], jdata['cid'])
      syslog.syslog(syslog.LOG_INFO, line)
      return True
    elif react == None:
      line = "Skipped healthy reaction type %s for monitor %s" % (redata['rtype'], jdata['cid'])
      syslog.syslog(syslog.LOG_INFO, line)
      return None
    else:
      line = "Error while processing healthy reaction type %s for monitor %s" % (redata['rtype'], jdata['cid'])
      syslog.syslog(syslog.LOG_INFO, line)
      return False
  else:
    line = "Got an unknown status for health check %s: %s" % (jdata['cid'], jdata['status'])
    syslog.syslog(syslog.LOG_ERR, line)
    return False
...
def getReaction(rid):
  ...
  if "lastrun" in cache:
    if cache['lastrun'] == None or cache['lastrun'] == "None":
