...
def logit(jdata, rdb, r_server):
  ...
  try:
    ...
    success = False
    cacheonly = True
    line = "logit-events: RethinkDB is inaccessible cannot log %s, sending to redis" % jdata['cid']
    ...
    try:
      ...
      line = "logit-events: Redis is inaccessible cannot log %s, via redis" % jdata['cid']
      syslog.syslog(syslog.LOG_INFO, line)
      success = False
  return success
