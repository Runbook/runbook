...
def healthy(redata, jdata, rdb, r_server):
  ...
  if jdata['check']['prev_status'] != "healthy":
    ...
    resetFailcount(jdata['cid'], rdb, r_server)
    if result:
      line = "chstatus: Setting monitor %s status to healthy" % jdata['cid']
      syslog.syslog(syslog.LOG_INFO, line)
      return True
    else:
...
def chStatus(cid, status, rdb, r_server):
  ...
  try:
    results = r.table('monitors').get(cid).update({'status': status}).run(rdb)
...
def resetFailcount(cid, rdb, r_server):
  ...
  ## Try to set rethinkdb first
  try:
    results = r.table('monitors').get(cid).update({'failcount': failcount}).run(rdb)
    if results['replaced'] == 1:
      success = True
      cacheonly = False
    else:
