...
def callDO(redata, jdata):
  ...
  if req.status_code >= 200 and req.status_code < 300:
    line = "digitalocean-shutdown: Reqeust to %s sent for monitor %s - Successful" % ( url, jdata['cid'] ) 
    syslog.syslog(syslog.LOG_INFO, line)
    return True
  else:
    line = "digitalocean-shutdown: Request to %s sent for monitor %s - Failed" % ( url, jdata['cid'] ) 
    syslog.syslog(syslog.LOG_INFO, line)
    return False
