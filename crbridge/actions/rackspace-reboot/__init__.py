...
def call_action(redata, jdata):
  ...
  try:
    req = requests.post(url, timeout=10.0, data=payload, headers=headers, verify=True)
    ...
    if req.status_code == 200:
      ...
      try:
        ...
        line = "rackspace-reboot: Failed Rackspace API Call for reaction %s"  % ( redata['id'] ) 
        syslog.syslog(syslog.LOG_INFO, line)
        return False
    else:
      line = "rackspace-reboot: Failed Rackspace Authenticaiton for reaction %s"  % ( redata['id'] ) 
      syslog.syslog(syslog.LOG_INFO, line)
      return False
  except:
    line = "rackspace-reboot: Failed Rackspace Authenticaiton for reaction %s"  % ( redata['id'] ) 
  ...
  if req.status_code == 202:
    ...
    line = "rackspace-reboot: Request to %s sent for monitor %s - Failed" % ( url, jdata['cid'] ) 
    syslog.syslog(syslog.LOG_INFO, line)
    return False
