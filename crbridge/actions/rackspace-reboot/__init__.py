...
def call_action(redata, jdata):
    ...
    try:
        ...
        if req.status_code == 200:
            ...
            try:
                ...
                line = "rackspace-reboot: Failed Rackspace API Call for reaction %s" % (
                    redata['id'])
                syslog.syslog(syslog.LOG_INFO, line)
                return False
        else:
            line = "rackspace-reboot: Failed Rackspace Authenticaiton for reaction %s" % (
                redata['id'])
            syslog.syslog(syslog.LOG_INFO, line)
            return False
    except:
    ...
    if req.status_code == 202:
        line = "rackspace-reboot: Reqeust to %s sent for monitor %s - Successful" % (
            url, jdata['cid'])
        syslog.syslog(syslog.LOG_INFO, line)
        return True
    else:
        line = "rackspace-reboot: Request to %s sent for monitor %s - Failed" % (
            url, jdata['cid'])
        syslog.syslog(syslog.LOG_INFO, line)
        return False
