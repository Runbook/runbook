  ...
  if failback == True:
    ...
    for rec in records:
      ...
      if rstring is not None:
        ...
        if result:
          ...
          line = "cloudflare-ip-replace: Record %s changed to IP %s" % (data['id'], data['content'])
    ...
    for rec in faileddata.keys():
      ...
      if result:
        ...
        line = "cloudflare-ip-replace: Failed to edit record %s" % data['id']
        syslog.syslog(syslog.LOG_DEBUG, line)
      runcount = runcount + 1
...
def callAPI(reqdata):
  ...
  try:
    req = requests.post(url="https://www.cloudflare.com/api_json.html", data=reqdata, headers=headers)
  except:
