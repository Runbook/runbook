...
def failed(redata, jdata, rdb, r_server):
  ...
  if run:
    ...
    if result:
      line = "enotify: Sent %s email notification for monitor %s" % (jdata['check']['status'], jdata['cid'])
    ...
    line = "enotify: Skipping %s email notification for monitor %s" % (jdata['check']['status'], jdata['cid'])
    syslog.syslog(syslog.LOG_ERR, line)
    return None
...
def emailNotify(redata, jdata, tfile):
  ...
  data = {}
  templateLoader = jinja2.FileSystemLoader( searchpath="/data/crbridge/templates/" )
