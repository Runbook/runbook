...
def remove(redata, jdata, r_server):
  ...
  failedrecs, faileddata = cloudflail.checkZone(redata['data']['ip'], usrdata)
  ...
  if numrecs > 0:
    ...
    for rec in failedrecs:
      ...
      if result:
        ...
        r_server.sadd('domains:' + jdata['cid'] + ':' + zone + ':' + rdata["content"] + ':deleted', rec)
    ...
    line = "cloudflare-ip-remove: IP %s not found in zone" % redata['data']['ip'] 
    syslog.syslog(syslog.LOG_DEBUG, line)
  return True
...
def readd(redata, jdata, r_server, replaceip=None):
  ...
  readds = len(r_server.smembers("domains:" + jdata['cid'] + ':' + zone + ":" + ip + ":deleted"))
  try:
    for item in range(0,readds + 1):
      deleted = r_server.spop("domains:" + jdata['cid'] + ':' + zone + ":" + ip + ":deleted")
      add_data = []
      ...
      if service_mode == "1":
        result = cloudflail.addRecord(add_data, usrdata, service_mode=True)
      else:
