...
def callDO(redata, jdata):
  ...
  url = "https://api.digitalocean.com/v2/droplets/%s/actions" % str(redata['data']['dropletid'])
  ...
  if req.status_code >= 200 and req.status_code < 300:
    line = "digitalocean-powercycle: Reqeust to %s sent for monitor %s - Successful" % ( url, jdata['cid'] ) 
    syslog.syslog(syslog.LOG_INFO, line)
    return True
  else:
    line = "digitalocean-powercycle: Request to %s sent for monitor %s - Failed" % ( url, jdata['cid'] ) 
    syslog.syslog(syslog.LOG_INFO, line)
    return False
