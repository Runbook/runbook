...
class ReactForm(BaseReactForm):  #pylint: disable=no-init
    ...
    description = """
    <p>This reaction will open an SSH connection to the specified host and remove a Docker container. If the container is currently running it will be stopped and then removed. While the container is being removed the image will still be present after execution of this reaction.</p>
