...
class MonitorTests(BaseTestCase):
    ...
    def test_user_can_view_monitor_history(self):
        # Ensure that user must be logged in to view monitor history.
        timestamp = str(datetime.datetime.now())
        with self.client:
            self.client.post(
                '/login',
            ...
            for result in results:
                ...
                break
            response = self.client.get(
                '/dashboard/view-history/{0}/0/200'.format(monitor_id),
                follow_redirects=True
            )
            self.assertEqual(response.status_code, 200)
            self.assertIn('Monitor History', response.data)
            self.assertIn(timestamp, response.data)

    def test_user_can_view_monitor_history_login(self):
        # Ensure in order to view the monitor history you must be logged in.
        timestamp = str(datetime.datetime.now())
        with self.client:
            self.client.post(
                '/login',
