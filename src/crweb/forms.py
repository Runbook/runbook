...
class SignupForm(Form):
    ...
    password = PasswordField(
        "Password",validators=[Length(min=8, message='Password must be a minimum of 8 characters')])
...
class LoginForm(Form):
    ...
    email = TextField(
        "Email", validators=[Email(message='Email address invalid')])
    password = PasswordField(
        "Password", validators=[DataRequired(message='Password is required to login')])
